...
const DEL_COLS = [
  ...
  { id: 'del_shipped',   label: 'Shipped',    color: ORANGE, dim: 'rgba(251,146,60,0.12)' },
];
      ...
      <div className="sheet" style={{
        ...
        <div style={{ overflow: 'auto', flex: 1, padding: '1.25rem 1.5rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
            <div>
              <div style={{ fontSize: '1.5rem', fontWeight: 900, color: T1, lineHeight: 1.2, marginBottom: '0.35rem' }}>
                {order.name}
              </div>
              <a href={`tel:${order.phone}`} style={{ fontSize: '1.1rem', fontWeight: 700, color: BLUE, textDecoration: 'none' }}>
                {order.phone}
              </a>
            </div>
            <div style={{ textAlign: 'right' }}>
              {isDelivery && order.woo_order_number && (
                <div style={{ fontFamily: MONO, fontSize: '0.75rem', color: T3, marginBottom: '0.35rem' }}>
                  Order #{order.woo_order_number}
                </div>
              )}
              <div style={{
                display: 'inline-flex', alignItems: 'center', gap: '0.4rem',
                background: col.dim, border: `1px solid ${col.color}44`,
                padding: '0.3rem 0.7rem', borderRadius: '20px',
              }}>
                <div style={{ width: 7, height: 7, borderRadius: '50%', background: col.color }} />
                <span style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '1.5px', textTransform: 'uppercase', color: col.color }}>
                  {col.label}
                </span>
              </div>
            </div>
          ...
          {isDelivery && fullAddress && (
            <div style={{
              background: BG, border: `1px solid ${BORDER}`, borderLeft: `4px solid ${BLUE}`,
              padding: '1rem 1.1rem', borderRadius: '8px', marginBottom: '1rem',
            }}>
              <div style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '2px', textTransform: 'uppercase', color: T3, marginBottom: '0.5rem' }}>
                📦 Delivery Address
              </div>
              <div style={{ fontSize: '1rem', fontWeight: 700, color: T1, lineHeight: 1.7, whiteSpace: 'pre-line' }}>
                {fullAddress}
              </div>
            </div>
          )}
          ...
          <div style={{
            ...
            <div style={{ fontSize: '1rem', fontWeight: 700, color: T1, lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
              {order.order_text}
            </div>
          </div>
          ...
          {!isDelivery && order.pickup && (
            <div style={{ marginBottom: '1rem' }}>
              <div style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '2px', textTransform: 'uppercase', color: T3, marginBottom: '0.35rem' }}>
                📅 Pickup Date
              </div>
              <div style={{ fontSize: '1.2rem', fontWeight: 900, color: AMBER }}>
                {formatPickup(order.pickup)}
              </div>
            </div>
          )}
          ...
          <div style={{ fontFamily: MONO, fontSize: '0.65rem', color: T3, marginBottom: '1.5rem' }}>
        ...
        <div style={{
          ...
          {nextCol && (
            <button className="btn-tap" onClick={() => { onMove(order.id, nextCol.id); onClose(); }}
              style={{
                height: 62, background: nextCol.color, color: '#000',
          ...
          <div style={{ display: 'flex', gap: '0.6rem' }}>
            <button className="btn-tap" onClick={() => { onMove(order.id, archiveColId); onClose(); }}
              style={{
                flex: 1, height: 54,
                background: 'none', border: `2px solid ${GREEN}55`, color: GREEN,
                borderRadius: '12px', fontFamily: FONT, fontSize: '0.85rem', fontWeight: 800,
                cursor: 'pointer',
              }}>
              ✓ Done
            </button>
            <button className="btn-tap" onClick={() => { if (window.confirm('Remove this order?')) { onDel(order.id); onClose(); } }}
              style={{
                flex: 1, height: 54,
                background: 'none', border: `2px solid ${RED}33`, color: RED,
                borderRadius: '12px', fontFamily: FONT, fontSize: '0.85rem', fontWeight: 800,
                cursor: 'pointer',
              }}>
              🗑 Delete
          ...
          <button className="btn-tap" onClick={onClose}
            style={{
              height: 48, background: 'none', border: `1px solid ${BORDER}`,
              color: T3, borderRadius: '12px', fontFamily: FONT,
    ...
    <div className="card btn-tap" onClick={onClick} style={{
      ...
      <div style={{ padding: '0.85rem 0.9rem 0.75rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.25rem' }}>
          <div style={{ fontWeight: 900, fontSize: '1rem', color: T1, lineHeight: 1.2, flex: 1, marginRight: '0.5rem' }}>
            {order.name}
          </div>
          <span style={{ fontFamily: MONO, fontSize: '0.65rem', color: T3, flexShrink: 0, paddingTop: '2px' }}>
            {timeAgo(order.created_at)}
          </span>
        </div>

        {/* Phone */}
        <div style={{ fontFamily: MONO, fontSize: '0.8rem', color: T2, marginBottom: '0.5rem' }}>
          {order.phone}
        </div>

        {/* Order preview — capped at 3 lines */}
        <div style={{
          fontSize: '0.85rem', fontWeight: 600, color: T1,
        ...
        {/* Footer: pickup date or suburb */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {isDelivery ? (
            <span style={{ fontSize: '0.75rem', color: BLUE, fontWeight: 700 }}>
              {suburb ? `📍 ${suburb}` : '📦 Delivery'}
            </span>
          ) : (
            <span style={{ fontSize: '0.75rem', fontWeight: 800, color: AMBER }}>
              {order.pickup ? `📅 ${formatPickup(order.pickup)}` : ''}
            </span>
          )}
          <span style={{ fontSize: '0.7rem', color: T3, fontWeight: 600 }}>
            Tap for details →
          </span>
        </div>
      </div>
...
function KanbanBoard({ orders, cols, archiveColId, archiveLabel, isDelivery, isMobile, dragOver, onDragOver, onDrop, onDragLeave, onDragStart, onTouchStart, onTouchMove, onTouchEnd, onMove, onDel, onCardTap }) {
  ...
  return (
    <div style={{ overflowX: 'auto', overflowY: 'visible', paddingBottom: '1rem' }}>
      <div style={{
        display: 'grid',
        gridTemplateColumns: isMobile ? '1fr' : `repeat(${colCount}, minmax(250px, 1fr))`,
        gap: '0.75rem',
        minWidth: isMobile ? 'auto' : `${colCount * 260}px`,
        padding: '0 1rem',
      }}>
        {cols.map(col => {
          ...
          return (
            <div key={col.id} data-col-id={col.id}
              ...
              style={{
                background: over ? SURFACE2 : `${col.dim}`,
                border: `1px solid ${over ? col.color + '44' : BORDER}`,
                borderRadius: '12px',
                overflow: 'hidden',
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', fontWeight: 900, letterSpacing: '1.5px', textTransform: 'uppercase', color: col.color }}>
                    {col.label}
                  </span>
                </div>
                {cards.length > 0 && (
                  <span style={{
                    fontFamily: MONO, fontSize: '0.85rem', fontWeight: 700, color: col.color,
              ...
              <div style={{ padding: '0.65rem', flex: 1 }}>
                {cards.length === 0 && (
                  <div style={{ textAlign: 'center', color: T3, fontSize: '0.7rem', letterSpacing: '1px', textTransform: 'uppercase', marginTop: '2rem' }}>
                    Empty
                  </div>
                )}
                {cards.map(o => (
        ...
        {/* Archive drop zone */}
        <div data-col-id={archiveColId}
          onDragOver={e => onDragOver(e, archiveColId)}
          onDrop={e => onDrop(e, archiveColId)}
          onDragLeave={onDragLeave}
          style={{
            border: `2px dashed ${dragOver === archiveColId ? T2 : BORDER}`,
            borderRadius: '12px',
            minHeight: isMobile ? 80 : 400,
            display: 'flex', flexDirection: 'column',
            background: dragOver === archiveColId ? SURFACE2 : 'transparent',
            transition: 'all 0.15s',
          }}>
          <div style={{ padding: '0.85rem 1rem', borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: 900, letterSpacing: '1.5px', textTransform: 'uppercase', color: T3 }}>
              {archiveLabel}
            </span>
...
function ArchiveList({ orders, onMove, onDel }) {
  ...
  const list = orders
    ...
    .filter(o => !q || [o.name, o.phone, o.order_text, o.city, o.woo_order_number].some(f => (f || '').toLowerCase().includes(q)))
    .sort((a, b) => b.created_at - a.created_at);

  const isDel = o => o.status === 'delivered';

  return (
    <div style={{ padding: '1rem' }}>
      <input type="text" placeholder="Search archived orders…" value={search}
      ...
      {list.length === 0 && (
        <div style={{ textAlign: 'center', color: T3, marginTop: '3rem', fontSize: '0.85rem', letterSpacing: '1px', textTransform: 'uppercase' }}>
      ...
      {list.map(o => (
        <div key={o.id} style={{
          ...
          borderLeft: `4px solid ${isDel(o) ? BLUE : AMBER}`,
          borderRadius: '10px', padding: '1rem 1.1rem', marginBottom: '0.6rem',
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '0.35rem' }}>
              ...
              <span style={{
                ...
                border: `1px solid ${isDel(o) ? BLUE : AMBER}44`,
                color: isDel(o) ? BLUE : AMBER, borderRadius: '20px', textTransform: 'uppercase', letterSpacing: '0.5px',
              }}>
                {isDel(o) ? 'Delivery' : 'Pickup'}
              </span>
            </div>
            <div style={{ fontSize: '0.85rem', color: T2, lineHeight: 1.5, marginBottom: '0.35rem',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', flexShrink: 0 }}>
            ...
            <button className="btn-tap" onClick={() => onDel(o.id)}
              style={{ background: 'none', border: 'none', color: T3, fontSize: '1.2rem', cursor: 'pointer', textAlign: 'center', borderRadius: '6px', padding: '0.2rem' }}>
              ×
            </button>
          </div>
        </div>
...
export default function App() {
        ...
        <div style={{ fontFamily: FONT, background: BG, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
          <div style={{ width: '100%', maxWidth: 380 }}>
            <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
              {LOGO_URL
                ? <img src={LOGO_URL} alt="Dry Age Biltong" style={{ height: 64, filter: 'invert(1)', width: 'auto' }} />
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: '16px', padding: '2rem' }}>
              <div style={{ fontSize: '0.7rem', fontWeight: 800, letterSpacing: '2.5px', textTransform: 'uppercase', color: T3, textAlign: 'center', marginBottom: '1.75rem' }}>
                Staff Login
              </div>
            ...
            <div style={{ textAlign: 'center', marginTop: '1.5rem' }}>
              <span onClick={() => setView('customer')} style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: T3, cursor: 'pointer' }}>
                ← Back to Order Form
              </span>
            </div>
          </div>
      ...
      <>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
            {LOGO_URL
              ? <img src={LOGO_URL} alt="" style={{ height: 36, filter: 'invert(1)', width: 'auto' }} />
              : <span style={{ fontSize: '0.9rem', fontWeight: 900, color: T1, letterSpacing: '2px', textTransform: 'uppercase' }}>DAB</span>
            }
          </div>
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button className="btn-tap" onClick={handleRefresh}
              style={{ height: 40, padding: '0 1rem', background: SURFACE, border: `1px solid ${BORDER}`, color: T2, fontFamily: FONT, fontSize: '0.7rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', borderRadius: '8px' }}>
              {refreshing ? '...' : '↻ Refresh'}
            </button>
            <button className="btn-tap" onClick={() => { setPin(''); setView('customer'); }}
              style={{ height: 40, padding: '0 1rem', background: 'none', border: `1px solid ${BORDER}`, color: T3, fontFamily: FONT, fontSize: '0.7rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', borderRadius: '8px' }}>
              Lock
            </button>
          </div>
        </div>
          ...
          {tabs.map(tab => {
            ...
            return (
              <button key={tab.id} className="btn-tap" onClick={() => setActiveTab(tab.id)}
                style={{
                  flex: 1, height: 60, background: 'none',
                  border: 'none', borderBottom: `3px solid ${active ? tab.color : 'transparent'}`,
                  cursor: 'pointer', fontFamily: FONT, padding: '0 0.5rem',
                  transition: 'border-color 0.15s',
                }}>
                <div style={{ fontSize: m ? '0.7rem' : '0.85rem', fontWeight: 900, color: active ? tab.color : T3, letterSpacing: '0.5px', textTransform: 'uppercase', marginBottom: '0.15rem' }}>
                  {tab.label}
                </div>
                {tab.count > 0 && (
                  <span style={{
                    fontFamily: MONO, fontSize: '0.75rem', fontWeight: 700,
                    color: active ? tab.color : T3,
                    background: active ? `${tab.color}22` : 'none',
                    padding: '0.05rem 0.45rem', borderRadius: '12px',
                  }}>
                    {tab.count}
                  </span>
                )}
        ...
        <div style={{ paddingTop: '0.85rem', paddingBottom: '2rem', minHeight: 'calc(100vh - 120px)' }}>
          {activeTab === 'pickup' && (
            <KanbanBoard
              cols={PICKUP_COLS} archiveColId="picked_up" archiveLabel="Picked Up"
              isDelivery={false} {...boardProps} />
          )}
          {activeTab === 'delivery' && (
            <KanbanBoard
              cols={DEL_COLS} archiveColId="delivered" archiveLabel="Delivered"
              isDelivery={true} {...boardProps} />
          )}
          {activeTab === 'archive' && (
        ...
        {/* ORDER DETAIL MODAL */}
        {selectedOrder && (() => {
          const isDelivery = DEL_STATUSES.has(selectedOrder.status) || selectedOrder.status === 'delivered';
        ...
        <div style={{ fontFamily: FONT, background: '#f9fafb', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem' }}>
          <div style={{ maxWidth: 440, width: '100%' }}>
            ...
            <div style={{ background: '#fff', border: '2px solid #111827', borderRadius: '16px', padding: m ? '1.5rem' : '2.5rem', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '0.9rem', color: '#6b7280', marginBottom: '1.75rem', lineHeight: 1.7 }}>
                We've received your order and sent you a confirmation SMS. We'll text you when it's ready for pickup.
              </p>
              <button className="btn-tap" onClick={() => setSuccess(false)}
                style={{ display: 'block', width: '100%', height: 54, background: '#111827', color: '#fff', border: 'none', borderRadius: '12px', fontFamily: FONT, fontSize: '0.9rem', fontWeight: 900, letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer' }}>
                Place Another Order
              </button>
            </div>
            <div style={{ textAlign: 'center', marginTop: '1.25rem' }}>
              <span onClick={() => setView('staff')} style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: '#9ca3af', cursor: 'pointer' }}>
                Staff View
              </span>
            </div>
          </div>
  ...
  // ── CUSTOMER FORM ────────────────────────────────────────────────────────────
  const inp = {
    display: 'block', width: '100%', height: m ? 48 : 52,
    border: '1.5px solid #d1d5db', background: '#fff', color: '#111827',
    fontFamily: FONT, fontSize: '1rem', fontWeight: 600,
    padding: '0 1rem', boxSizing: 'border-box', outline: 'none',
    borderRadius: '10px', appearance: 'none', WebkitAppearance: 'none',
  };
  const lbl = {
    display: 'block', fontSize: '0.7rem', fontWeight: 800,
    letterSpacing: '1.5px', textTransform: 'uppercase',
    marginBottom: '0.45rem', color: '#374151',
  };
    ...
    <>
      ...
      <div style={{ fontFamily: FONT, background: '#f3f4f6', minHeight: '100vh', padding: m ? '1rem' : '2.5rem 1rem' }}>
        <div style={{ maxWidth: 500, margin: '0 auto' }}>
          {LOGO_URL && (
            <div style={{ textAlign: 'center', marginBottom: m ? '1rem' : '2rem' }}>
              <img src={LOGO_URL} alt="Dry Age Biltong" style={{ height: m ? 48 : 80, width: 'auto' }} />
            </div>
          )}
          <div style={{ background: '#fff', border: '1.5px solid #e5e7eb', borderRadius: '16px', padding: m ? '1.25rem' : '2.25rem' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ fontSize: '0.75rem', color: '#9ca3af', marginBottom: '0.4rem' }}>Include quantity in kg (e.g. 2kg biltong, 1kg boerewors)</div>
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <input type="date" style={{ ...inp, borderColor: errors.pickup ? '#ef4444' : '#d1d5db' }} value={form.pickup} min={today}
            ...
            {Object.keys(errors).filter(k => k !== 'order_text').length > 0 && (
              <div style={{ fontSize: '0.8rem', color: '#ef4444', fontWeight: 700, marginBottom: '0.75rem' }}>Please fill in all required fields.</div>
            )}
          ...
          <div style={{ textAlign: 'center', marginTop: '1.25rem', paddingBottom: '1.5rem' }}>
            <span onClick={() => setView('staff')} style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: '#9ca3af', cursor: 'pointer', userSelect: 'none' }}>
              Staff View
            </span>
          </div>
        </div>
